...
export async function assertCanUploadColaboradorFoto(
  ...
  if (!canCrudColaboradores(tipoAcesso, permissoesAdmin ?? undefined)) {
...
export async function assertCanUploadVeiculoContrato(
  request: Request
): Promise<void> {
  const token = await tokenFromRequest(request);
  if (!token) {
  ...
  const tipoAcesso = jwtTipoAcesso(token) as TipoAcessoSistema | null;
  const permissoesAdmin = jwtPermissoesAdmin(token);
  if (!canFrotaVisualizarContratos(tipoAcesso, permissoesAdmin ?? undefined)) {
    throw new Response(
      JSON.stringify({ error: "Sem permissão para enviar contrato." }),
      {
        status: 403,
        headers: { "Content-Type": "application/json" },
      }
